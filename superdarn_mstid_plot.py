...
# https://colorcet.holoviz.org/user_guide/Continuous.html
prmd['cmap']            = mpl.colormaps['cet_CET_C6'] #colorcet.cyclic_rygcbmr_50_90_c64
...
def native(obj):
    """
    Convert NumPy scalars to their plain Python equivalents.

    netCDF attributes come back from xarray as NumPy scalars. NumPy 2 changed their repr
    from "0.25" to "np.float64(0.25)", which leaks into the attribute dictionaries printed
    verbatim into the CSV headers written by write_csv() and calculate_reduced_index().
    Converting on load keeps those headers stable across NumPy versions.
    """
...
class ParameterObject(object):
    def __init__(self,param,radars,seasons=None,
        ...
        try:
            ...
            min_orig_rti_fraction   = float(np.nanmin(orf))
            ...
            for season in seasons:
                ...
                min_orf = float(np.nanmin(orf.values))
            ...
            for season in seasons:
                orf     = orig_rti_fraction[season]['df']
                for radar in radars:
                    min_orf = float(np.nanmin(orf[radar].values))
    ...
    def calculate_reduced_index(self,season,
        ...
        if reduction_type == 'median':
            red_vals    = np.nanmedian(data_arr,axis=1)
        elif reduction_type == 'mean':
...
#        reducedIndex['smoothed']    = reducedIndex['reduced_index'].rolling(smoothing_window,center=True).mean()
        ...
        if write_csvs:
            ...
            with open(csv_fpath,'w') as fl:
                ...
                hdr.append('# Generated on: {!s} UTC'.format(datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)))
                hdr.append('#')
    ...
    #            fl  = os.path.join(data_dir,'sdMSTIDindex_{!s}_{!s}.nc'.format(season,radar))
                ...
                ds.append(dsr)
                attrs_radars[radar] = native(dsr.attrs)

                # Store radar lat / lons to creat a radar location file.
                lat_lons.append({'radar':radar,'lat':native(dsr.attrs['lat']),'lon':native(dsr.attrs['lon'])})
            ...
            dss   = xr.concat(ds,dim='index',join='outer')
